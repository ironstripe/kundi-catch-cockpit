...
- `src/lib/catch-calculation.ts`: `CalculationInput.internal_handling_cost_per_unit?: number | null`; `CalculationValues` erhält `internal_handling_cost_per_unit`, `internal_handling_cost_total`, `internal_handling_cost_share_percentage`, `db_i` (Alias auf `maximum_contribution_margin`), `db_ii`, `db_ii_margin_percentage` (alle `null`, wenn kein Satz erfasst ist). Bestehende Felder, Schwellenwertlogik und Ampel bleiben unangetastet — DB II und der Anteil beeinflussen die Entscheidungsstufe nicht.
- `src/lib/catch-reconciliation.ts`: gleiche Zusatzfelder in `ReconciliationValues` inkl. `internal_handling_cost_share_percentage` auf Basis des effektiven Nettoerlöses, plus Aggregation in `HistoryTotals` (`internal_handling_cost`, `contribution_margin_ii`).
...
- `src/components/catch/calculation-card.tsx`: neuer Abschnitt DB I / interner Aufwand (Total mit Anteil in % des Nettoerlöses) / DB II / DB-II-Marge mit Erklärsatz «DB II berücksichtigt den direkt zurechenbaren internen Catch-Aufwand, jedoch keine allgemeinen Betriebs- oder Gemeinkosten.»
- `src/components/catch/reconciliation-card.tsx`, `completed-summary.tsx`, `src/routes/_authenticated/history.tsx`, `src/lib/export-workbook.ts`: effektive DB-II-Werte und Anteil ergänzen, bestehende Spalten erhalten.
- `src/lib/sounding.ts`: `SoundingSnapshot` um `internal_handling_cost_per_unit`, `internal_handling_cost_total`, `internal_handling_cost_share_percentage`, `db_ii`, `db_ii_margin_percentage`; Teams-Text erhält kompakte Zeilen für DB I, internen Aufwand mit Anteil und DB II. Alte Snapshots ohne diese Felder werden tolerant als «nicht erfasst» gelesen.
